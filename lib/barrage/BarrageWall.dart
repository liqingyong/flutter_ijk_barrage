...
//              '${_lastBullets[nextChannel]?.id} == ${context.hashCode} $child pos: ${animation.value}');
          ...
          //计算控件宽度
          if(child is Text) {
            widgetWidth = boundingTextSize(context,child.data!,child.style??TextStyle(color: Colors.white,fontSize: 18)).width;
          }
         /* Bullet bullet = child as Bullet;
          if (bullet.child is Text) {
            Text text = bullet.child as Text;
            widgetWidth = boundingTextSize(context,text.data!,text.style!).width;
          }*/
